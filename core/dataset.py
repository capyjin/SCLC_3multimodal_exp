...
def build_fold_test_dataset(clinical_frame, corpus, standardize_cols, categorical_cols, ids,
                            target, image_dir, resize=512, gray_scale=True,
                            tfidf_max_features=400, tfidf_ngram_range=(2, 4)):
    """저장된 체크포인트를 fold의 test 환자에 다시 태우기 위한 재구성 헬퍼.

    학습 때와 **똑같은 경로**로 (a) train fold 만으로 fit 한 결합 텐서와
    (b) train fold 픽셀 통계로 정규화한 test Dataset 을 만든다. 사후 분석
    스크립트(기여도·중복도)가 각자 이 20여 줄을 복사해 갖고 있었는데, 한쪽만
    고치면 두 분석이 서로 다른 전처리를 쓰게 되므로 여기로 합쳤다.

    Returns ``(test_dataset, clinical_dim, report_dim)``.
    """
    tabular, clinical_dim, report_dim = features.build_fold_multimodal_tabular(
        ...
        tfidf_max_features=tfidf_max_features, tfidf_ngram_range=tfidf_ngram_range,
    )
    train_samples = preprocess_data(image_dir, clinical_frame.loc[ids["train"]].reset_index(), target, gray_scale)
    test_samples = preprocess_data(image_dir, clinical_frame.loc[ids["test"]].reset_index(), target, gray_scale)
    _, test_ds = create_dataset(train_samples, test_samples, resize, False, gray_scale, False,
                                train_tabular=tabular["train"], test_tabular=tabular["test"])
    return test_ds, clinical_dim, report_dim
