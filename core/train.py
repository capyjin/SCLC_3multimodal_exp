# -*- coding: utf-8 -*-
"""Cox 손실 · 학습 루프 · 이미지가 관여하는 모델의 K-fold 평가기.
...
arm 은 반대로 pycox ``CoxPH`` + torchtuples 를 쓴다
(실험1의 ``late_fusion_3modal.py``).

공개 API: ``cox_ph_loss``, ``evaluate_c_index``, ``fit``, ``fold_plan``,
``seed_everything``, ``TrimodalEvaluator``, ``ImageOnlyEvaluator``.
``fold_plan``/``seed_everything`` 은 여러 실험이 직접 가져다 쓰므로 밑줄 없는
공개 이름이다(예전 ``_fold_plan``/``_seed_everything``).
"""
import os
import random
import shutil
...
from core.model import ConcatDeepSurv, ImageOnlyDeepSurv
...
def train_one_epoch(model: nn.Module, loader: DataLoader, optimizer: optim.Optimizer, device: torch.device,
    ...
    for batch in pbar:
        ...
        loss.backward()
        if grad_modulator is not None:
            grad_modulator(model, images, tabular, durations, events)
        optimizer.step()
...
def fit(model, train_loader, val_loader, optimizer, device, epochs, save_path, grad_modulator=None):
    os.makedirs(os.path.dirname(save_path) or ".", exist_ok=True)
    best_val_cindex = -np.inf
    history: list[dict] = []

    for epoch in tqdm(range(1, epochs + 1), desc="Epochs"):
        train_loss = train_one_epoch(model, train_loader, optimizer, device, grad_modulator)
...
def fold_plan(cohort_df: pd.DataFrame, max_folds: int | None = None):
...
class TrimodalEvaluator:
    ...
    def __init__(
        ...
        model_factory=None, grad_modulator=None, text_source: str = "concl_find",
        extra_numeric_fn=None, text_encoder_fn=None, fix_brain_meta: bool = True,
        clinical_columns_fn=None,
    ):
        self.target = target
        self.model_factory = model_factory
        self.grad_modulator = grad_modulator
        # extra_numeric_fn: 임상 블록 뒤에 붙일 추가 숫자 feature 생성기
        # (features.build_fold_multimodal_tabular 참고). None이면 기존 동작과 완전히 동일.
        # fold마다 train id로만 fit 해야 하는 통계는 이 콜러블 안에서 처리한다.
        self.extra_numeric_fn = extra_numeric_fn
        # text_encoder_fn: report 브랜치에 들어갈 텍스트 블록을 TF-IDF **대신** 만드는 생성기
        # (features.build_fold_multimodal_tabular 참고). None이면 기존 TF-IDF 동작과 완전히 동일.
        # frozen BERT 임베딩 실험용 훅이며, fold별 통계(SVD/scaler)는 이 콜러블 안에서
        # train id로만 fit 해야 한다 (bert_features.make_text_encoder_fn).
        self.text_encoder_fn = text_encoder_fn
        # fix_brain_meta: brain_meta 를 기저 시점 전이만 1 로 다시 코딩(누수 수정).
        # 기본 True = 누수 수정본. False 면 2026-08-02 이전 결과 재현.
        self.fix_brain_meta = fix_brain_meta
        # clinical_columns_fn: 임상 블록에 쓸 컬럼 목록을 정하는 콜러블
        # ``(df) -> (standardize_cols, categorical_cols)``. None 이면 기존
        # ``features.resolve_clinical_columns`` 가 그대로 쓰여 동작이 완전히 동일하다.
        # 전체-중앙값으로 미리 대치된 컬럼을 임상 블록에서 **빼고** 같은 지표를
        # ``extra_numeric_fn`` 으로 fold-safe 하게 다시 넣을 때 쓴다
        # (실험7의 fold_safe_features.reduced_clinical_columns 참고) —
        # 둘 중 하나만 적용하면 같은 지표가 두 번(대치본+fold-safe본) 들어가므로
        # 반드시 짝으로 써야 하고, fold_safe_features 쪽 assert 가 이를 강제한다.
        self.clinical_columns_fn = clinical_columns_fn
        # text_source: 판독지 텍스트의 어느 부분을 쓸지 (features.load_text_corpus 참고).
        # 기본값 "concl_find" 가 기존 동작이라 기존 실험 결과는 그대로 재현된다.
    ...
    def run(self) -> "TrimodalEvaluator":
        cohort_df = cohort.load_trimodal_cohort(self.merged_csv, self.split_csv,
                                               fix_brain_meta=self.fix_brain_meta)
        clinical_frame = cohort_df.drop_duplicates("research_id").set_index("research_id")
        resolve_columns = self.clinical_columns_fn or features.resolve_clinical_columns
        ...
        for plan_index, (fold, ids) in enumerate(plan):
            ...
            fold_history = fit(model, train_loader, val_loader, optimizer, self.device, self.epochs, save_path,
                               grad_modulator=self.grad_modulator)
