# -*- coding: utf-8 -*-
"""왜 late fusion에서 이미지가 OS는 올리는데 PFS는 못 올리나?

late fusion의 최종 결합은 CoxPH 2변수 회귀:
    log h(t) = beta_tab * (tabular 위험점수) + beta_img * (이미지 위험점수)

따라서 "이미지가 도움이 되는가"는 beta_img가 0과 유의하게 다른가로 직접 검정할 수 있다.
저장된 체크포인트에서 OOF 위험점수를 재생성한 뒤 아래를 계산한다.

  ① beta_img의 fold별 값 + 95% 신뢰구간 + p-value      -> 신호가 안정적인가?
  ② 우도비 검정 (tabular 단독 vs tabular+image)         -> 이미지가 정보를 더하는가?
  ③ tabular 위험점수 vs 이미지 위험점수 상관             -> 중복인가?
  ④ tabular가 틀린 환자쌍에서 이미지의 C-index          -> 보완적인가?

Run:  python analyze_late_fusion_pfs.py
"""
import json
import os
...
def oof_scores(target):
    ...
    tab = TrimodalEvaluator(
        target=target, epochs=0, batch_size=32,
        save_dir=os.path.join(OUT_DIR, f"tabular_{target}"),
        model_factory=ablation.make_factory(ablation.CONFIGS["clin_report"]), seed=42,
    ).run()
    img = ImageOnlyEvaluator(
        target=target, epochs=0, batch_size=16, resize=512,
        save_dir=os.path.join(OUT_DIR, f"image_simplecnn_{target}"),
        ckpt_tag="image_simplecnn", seed=42,
    ).run()
    out = {
        "tabular": {r["research_id"]: r["risk_score"] for r in tab.oof_predictions},
        "image": {r["research_id"]: r["risk_score"] for r in img.oof_predictions},
    }
...
def analyze(target):
    ...
    rows = []
    for fold, ids in _fold_plan(df_all, max_folds=None):
        tr = frame(ids["train"])
